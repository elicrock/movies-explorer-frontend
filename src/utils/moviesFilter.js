...
export const filterMovies = (movies, keyword, isChecked) => {
  const lowercaseKeyword = keyword.toLowerCase();
  return movies.filter((movie) => {
    const nameRU = movie.nameRU.toLowerCase();
    const nameEN = movie.nameEN.toLowerCase();
    return (nameRU.includes(lowercaseKeyword) || nameEN.includes(lowercaseKeyword)) && (!isChecked || movie.duration <= 40);
  });
};